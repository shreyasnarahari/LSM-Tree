// Package lsmtree implements a production-grade Log-Structured Merge Tree
// key-value storage engine using only the Go standard library.
package lsmtree
...
const (
	// walHeaderSize is the fixed byte count of every WAL record header.
	//
	// Layout:
	...
	//   ─────────────────────────────
	//   Total     19 bytes
	walHeaderSize = 4 + 8 + 1 + 2 + 4 // 19

	// walBufSize is the user-space buffer size for bufio.Writer.
...
//
// Writes are batched through a bufio.Writer (4 KB buffer) to reduce the
// number of write(2) syscalls.  Durability is achieved by an explicit
// Sync() call that flushes the user-space buffer and then invokes
// fsync(2) on the underlying file descriptor.
type WAL struct {
	file   *os.File
	writer *bufio.Writer
	path   string
...
// OpenWAL creates or opens an existing WAL file at path.
//
...
// NewWALIterator opens the WAL file at path for recovery replay.
//
...
func NewWALIterator(path string) (*WALIterator, error) {
	...
	return &WALIterator{
		file:   f,
		reader: bufio.NewReaderSize(f, walBufSize),
	}, nil
}
...
func (it *WALIterator) truncateAt(offset int64) error {
