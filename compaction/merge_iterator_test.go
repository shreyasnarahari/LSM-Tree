...
func TestMergeIteratorSimple(t *testing.T) {
	...
	// Because NewMergeIterator expects iterator.Iterator, and mockIterator implements it
	// we need to pass them in an interface slice
	iters := []iterator.Iterator{it1, it2}
...
func TestMergeIteratorMultiShadow(t *testing.T) {
	it1 := &mockIterator{entries: []mockEntry{{Key: []byte("a"), Value: []byte("1")}}}
	it2 := &mockIterator{entries: []mockEntry{{Key: []byte("a"), Value: []byte("2")}}}
	it3 := &mockIterator{entries: []mockEntry{{Key: []byte("a"), Value: []byte("3")}}}

	iters := []iterator.Iterator{it1, it2, it3}
	merge := NewMergeIterator(iters)
