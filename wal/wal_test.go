...
// testWALPath returns a fresh WAL file path inside t.TempDir().
func testWALPath(t testing.TB) string {
	t.Helper()
...
func replayAll(t testing.TB, path string) []*Entry {
	t.Helper()
...
func TestWALCorruptCRC(t *testing.T) {
	...
	// Key starts at lastRecordStart + HeaderSize.
	corruptOffset := lastRecordStart + HeaderSize + 2
...
func TestWALCRC32CIntegrity(t *testing.T) {
	...
	if len(raw) != HeaderSize+len(key)+len(val) {
		t.Fatalf("file size = %d, want %d", len(raw), HeaderSize+len(key)+len(val))
	}
...
func BenchmarkWALAppendBatch(b *testing.B) {
	...
	recordBytes := int64(HeaderSize + len(key) + len(val))
